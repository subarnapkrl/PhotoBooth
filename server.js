...
const upload=multer();//Multer package is only used when user sends files 
let db;
...
//When App first launches, make sure the public/uploaded-photos exist
...
function passwordProtected(req,res,next){
    ...
    if(req.headers.authorization=="Basic YWRtaW46YWRtaW4xMjM="){
        next()
    }else{
...
app.get("/",async (req,res)=>{
    ...
    const generatedHTML=ReactDOMServer.renderToString(
        <div className="container">
            {!allAnimals.length && <p>There are no animals!</p>}
...
{
    const client=new MongoClient("mongodb+srv://mongodbUser:mongodb123@cluster0.mg233e3.mongodb.net/AmazingMernApp");
